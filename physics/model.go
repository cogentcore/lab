...
type Model struct {
	...
	// ReplicasN is the number of body elements within each set of
...
func (ml *Model) NewBody(shape Shapes, size, pos math32.Vector3, rot math32.Quat) int32 {
	...
	SetBodyObject(idx, int32(ml.CurrentObject))
	ml.SetMass(idx, shape, size, 0) // assume static
	return idx
}

// NewDynamic adds a new dynamic body with given parameters. Returns the index.
// Shape cannot be [Plane].
func (ml *Model) NewDynamic(shape Shapes, mass float32, size, pos math32.Vector3, rot math32.Quat) (bodyIdx, dynIdx int32) {
	if shape == Plane {
		panic("physics.NewDynamic: shape cannot be Plane")
	}
	bodyIdx = ml.NewBody(shape, size, pos, rot)
	...
	ml.SetMass(bodyIdx, shape, size, mass)
	return
}
