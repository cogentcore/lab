...
fn SetBroadContactA(idx: i32,bodIdx: i32) {
	BroadContacts[Index2D(TensorStrides[80], TensorStrides[81], u32(idx), u32(ContactA))] = bitcast<f32>(u32(bodIdx));
}
fn SetBroadContactB(idx: i32,bodIdx: i32) {
	BroadContacts[Index2D(TensorStrides[80], TensorStrides[81], u32(idx), u32(ContactB))] = bitcast<f32>(u32(bodIdx));
}
fn SetBroadContactPointIdx(idx: i32,ptIdx: i32) {
	BroadContacts[Index2D(TensorStrides[80], TensorStrides[81], u32(idx), u32(ContactPointIdx))] = bitcast<f32>(u32(ptIdx));
}
fn CollisionBroad(i: u32) { //gosl:kernel
	...
	var biA = BodyCollidePairs[Index2D(TensorStrides[50], TensorStrides[51], u32(ci), u32(0))];
	var biB = BodyCollidePairs[Index2D(TensorStrides[50], TensorStrides[51], u32(ci), u32(1))];
