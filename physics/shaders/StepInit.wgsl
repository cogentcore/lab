...
//////// import: "step.go"
fn StepInit(i: u32) { //gosl:kernel read-write:Params
	if (i > 0) {
		return;
	}
	var params = Params[0];
	...
	for (var j=0; j<params.JointDoFsN; j++) {
		var tpos = JointControls[Index2D(TensorStrides[110], TensorStrides[111], u32(j), u32(JointTargetPos))];
		var tcur = JointControls[Index2D(TensorStrides[110], TensorStrides[111], u32(j), u32(JointTargetPosCur))];
		if (abs(tpos-tcur) < params.ControlDtThr) {
			tcur = tpos;
		} else {
			tcur += params.ControlDt * (tpos - tcur);
		}
		JointControls[Index2D(TensorStrides[110], TensorStrides[111], u32(j), u32(JointTargetPosCur))] = tcur;
	}
	Params[0] = params;
}
