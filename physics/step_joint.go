...
func StepSolveJoints(i uint32) { //gosl:kernel
	...
	if jt == Distance {
		...
		for dim := range 3 {
			...
			// compute gradients
			linearc := math32.Vec3(frame_p.Value(int(0), int(dim)), frame_p.Value(int(1), int(dim)), frame_p.Value(int(2), int(dim)))
...
func UpdateJointAxisWeightedTarget(axis math32.Vector3, target, weight float32, axisTargets, axisWeights *math32.Vector3) {
	weightedAxis := axis.MulScalar(weight)
	*axisTargets = axisTargets.Add(weightedAxis.MulScalar(target)) // weighted target (to be normalized later by sum of weights)
