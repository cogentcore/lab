...
func (st *State) GenKernelHeader(sy *System, kn *Kernel, avars, tvars map[string]*Var) string {
	...
	for gi, gp := range sy.Groups {
		...
		for _, vr := range gp.Vars {
			_, isAtomic := avars[vr.Name]
			_, isUsed := tvars[vr.Name]
			if vr.Tensor && !isUsed && !isAtomic {
				continue
			}
			access := ", read_write"
	...
	b.WriteString("}\n")
	b.WriteString(st.GenTensorFuncs(sy, avars, tvars))
	return b.String()
}

// GenTensorFuncs returns the generated WGSL code
// for indexing the tensors in given system.
func (st *State) GenTensorFuncs(sy *System, avars, tvars map[string]*Var) string {
	var b strings.Builder
